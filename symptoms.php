...
$w = (filter_input ( INPUT_POST, 'getSymptom', FILTER_SANITIZE_STRING )) ? filter_input ( INPUT_POST, 'getSymptom', FILTER_SANITIZE_STRING ) : "";
?>
                ...
                <div style="">
                    <?php
																	foreach ( $letterList as $let ) {
																		...
																		$stmt = $db->prepare ( "SELECT  * FROM symptomIndex WHERE indexItem LIKE '$let%' ORDER BY indexItem" );
																		$stmt->execute ();
																		while ( $row = $stmt->fetch () ) {
																			$symId = $row ['id'];
																			$symptom = $row ['indexItem'];
																			echo "<div style='font-weight:bold; padding:5px;'><a name='$symId'>$symptom</a> - ";
																			$substmt = $db->prepare ( "SELECT id,name FROM herbData WHERE dataSheet LIKE '%$symptom%' ORDER BY name" );
																			$substmt->execute ();
																			$t = 1;
																			while ( $subrow = $substmt->fetch () ) {
																				...
																				echo "<a href='herbData.php?id=" . $subrow ['id'] . "&word=$symptom' style='color:#008000;'>" . $subrow ['name'] . "</a>";
																				$t ++;
																			}
																			echo "</div>";
																		}
...
<?php
																	...
																	$substmt = $db->prepare ( "SELECT id,name FROM herbData WHERE dataSheet LIKE '%$w%' ORDER BY name" );
																	$substmt->execute ();
																	$t = 1;
																	while ( $subrow = $substmt->fetch () ) {
																		if ($t != 1) {
